...
interactive = True
headers = ['index1', 'ship_nr', 'id', 'repeat_indicator', 'mmsi', 'nav_status', 'rot_over_range', 'rot', 'sog',
           'position_accuracy', 'x', 'y', 'cog', 'true_heading',
           'timestamp', 'special_manoeuvre', 'spare', 'raim', 'sync_state', 'slot_timeout', 'slot_offset', 'abs_time',
           'date', 'time']
...
def load_all_data(timesteps, dim, features, CLASSES):
    ...
    int(sum(data_all_tracks[:nr_of_vessels]) + timesteps * len(data_all_tracks) + data.shape[0]), column_len),
                           fill_value=-1.0)
    ...
    for mmsi in data1[:nr_of_vessels]:  # [vessel_nr:vessel_nr+1]: #
        ...
        if decoded_mmsi.shape[0] > timesteps:
            ...
            overall_data[startIndex + data_per_track: startIndex + data_per_track + decoded_mmsi.shape[0],
            0:dim] = decoded_mmsi.iloc[:, 2:6]
