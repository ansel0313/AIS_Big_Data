...
def get_tra_part(df):
    ...
    for idx in df.id.drop_duplicates():
        ...
        tra[idx] = [Point(t.LON[i], t.LAT[i]) for i in range(0, t.shape[0])] #key = mmsi, value = list of lon and lat
        line_seg[idx] = approximate_trajectory_partitioning(tra[idx], theta=0, traj_id=int(idx))
        seg.extend(line_seg[idx]) # [Seg_mmsi1(dict), Seg_mmsi2(dict), Seg_mmsi3(dict),...]
